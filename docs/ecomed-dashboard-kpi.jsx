...
// ═══════════════════════════════════════
const DATA = {
  northStar: { current: 14, target: 10, label: "Descartes/semana" },
  users: { total: 87, dau: 23, wau: 52, newWeek: 12, target: 100 },
  retention: { d7: 24, d30: 11 },
  coins: { total: 4280, avg: 49, spent: 340, rewards: 12 },
  disposals: { total: 42, week: 14, gps: 28, points: 8 },
  education: { articles: 218, quizzes: 156, chat: 387, quizAvg: 72 },
  levels: { semente: 52, broto: 28, arvore: 6, guardiao: 1, lenda: 0 },
  streaks: { avg: 4.2, best: 18, active: 34 },
  missions: { completed: 42, rate: 38 },
  tech: { uptime: 99.4, lighthouse: 92, api: 1.2, chat: 3.8, fcp: 1.3, tests: 48 },
  social: { ig: 340, tt: 180, li: 72, refs: 18, shares: 45, nps: 7.8 },
  impact: { liters: 18900000, people: 605, points: 53, cities: 4 },
  tasks: { total: 196, done: 34, progress: 28, review: 8, blocked: 5, notStarted: 121 },
  weekly: [
    { w: "S1", u: 12, d: 2, c: 340, a: 18 },
    { w: "S2", u: 28, d: 5, c: 890, a: 42 },
    { w: "S3", u: 45, d: 12, c: 1650, a: 78 },
    { w: "S4", u: 62, d: 24, c: 2800, a: 134 },
    { w: "S5", u: 74, d: 33, c: 3600, a: 178 },
    { w: "S6", u: 87, d: 42, c: 4280, a: 218 },
  ],
};
...
const statusColor = (v, t, invert = false) => {
  const r = invert ? t / v : v / t;
  if (r >= 1) return "#24A645";
  if (r >= 0.7) return "#D97706";
  return "#DC2626";
};
const statusLabel = (v, t, invert = false) => {
  const r = invert ? t / v : v / t;
...
function MetricCard({ label, value, unit, icon, target, invert, small, color: customColor }) {
  const c = target ? statusColor(value, target, invert) : customColor || colors.tealDark;
  return (
      ...
      {target && (
        <div style={{ marginTop: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontSize: 10, color: c, fontWeight: 600 }}>{statusLabel(value, target, invert)}</span>
            <span style={{ fontSize: 10, color: colors.gray }}>{pct(value, target)}% de {fmt(target)}</span>
          </div>
          <div style={{ height: 5, background: "#F3F4F6", borderRadius: 3, overflow: "hidden" }}>
            <div style={{ width: `${pct(value, target)}%`, height: "100%", background: c, borderRadius: 3, transition: "width 0.8s ease" }} />
          </div>
        </div>
      )}
    </Card>
...
function SparkBar({ data, color, height = 50 }) {
...
function LevelDistribution({ levels, total }) {
  const items = [
      ...
      <Card style={{ background: colors.tealDark, border: "none", marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <span style={{ fontSize: 12, color: "rgba(255,255,255,0.6)" }}>Meta MVP: {d.northStar.target}/semana — {statusLabel(d.northStar.current, d.northStar.target)}</span>
          </div>
          <div style={{ width: 160 }}>
  ...
  return (
    <>
      <Card style={{ marginBottom: 16, textAlign: "center" }}>
        ...
        <FlywheelDiagram data={d.social.nps ? { attract: { conversion: 16.7 }, engage: { streakAvg: d.streaks.avg }, delight: { referralRate: 20.7, nps: d.social.nps } } : {}} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        ...
        <Card>
          ...
          <div style={{ marginTop: 12, paddingTop: 10, borderTop: `1px solid ${colors.border}` }}>
            <div style={{ fontSize: 11, color: colors.gray, marginBottom: 4 }}>Velocidade: <span style={{ fontWeight: 700, color: "#1A1A1A" }}>{Math.round(d.tasks.done / 6)}</span> tarefas/semana</div>
            <div style={{ fontSize: 11, color: colors.gray }}>Estimativa p/ conclusão: <span style={{ fontWeight: 700, color: "#1A1A1A" }}>{Math.round((d.tasks.total - d.tasks.done) / (d.tasks.done / 6))}</span> semanas restantes</div>
          </div>
        </Card>
      </div>
...
export default function EcoMedDashboard() {
  const [tab, setTab] = useState("overview");

  const content = useMemo(() => {
    switch (tab) {
      case "overview": return <TabOverview />;
      case "flywheel": return <TabFlywheel />;
      case "coins": return <TabEcoCoins />;
      case "tech": return <TabTechnical />;
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", background: colors.bg, minHeight: "100vh" }}>
      ...
      <div style={{ background: colors.tealDark, padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}>Atualizado: {new Date().toLocaleDateString("pt-BR")} {new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</span>
      ...
      <div style={{ padding: "20px 24px", maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ marginTop: 32, paddingTop: 16, borderTop: `1px solid ${colors.border}`, textAlign: "center" }}>
          <p style={{ fontSize: 11, color: "#999" }}>
            EcoMed — Painel de KPIs v1.0 | Dados simulados (conectar Supabase para dados reais) | ecomed.eco.br
          </p>
        </div>
      </div>
    </div>
